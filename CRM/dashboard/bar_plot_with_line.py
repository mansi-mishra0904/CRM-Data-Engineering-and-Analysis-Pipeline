...
def plot_dual_axis_bar_line(data_pd, x_labels, bar_col, line_col, bar_label, line_label, title, x_title, bar_y_title, line_y_title, cmap='Greens', line_color='orange', figsize=(14, 6), rotation=45):
    ...
    ax1.set_ylabel(bar_y_title, color='Green', fontsize=14)
...
def plot_world_map(df, locations_col, color_col, plot_title, hover_data=None, is_discrete=True, color_scale=None, color_discrete_sequence=None):
    ...
    fig.update_layout(
        ...
        autosize=False,
        width=900,  # Set width for larger plot
        height=600   # Set height for larger plot
    )
...
def plot_trend(df, x_col, y_col, x_ticks=None, x_labels=None, title='', xlabel='', ylabel='', marker='^', linestyle='-', color='Green', figsize=(10, 4)):
    ...
    Parameters:
        ...
        color (str): Color for the plot line. Default is 'Green'.
...
def plot_performers_bar_chart(df, title, xlabel, ylabel, cmap='Greens', color_inversion=False):
    ...
    for bar in bars:
        ...
        plt.text(
            bar.get_x() + bar.get_width() / 2,
            yval + 0.01,
            # f'{yval}',  # Show values up to 3 decimal places
            va='bottom',
            ha='center',
            fontsize=10,
...
def plot_region_performance(region_performance_df):
    ...
    # Step 3: Define a colormap for different shades of Green
    cmap = get_cmap('Greens')
...
def plot_sales_rep_performance_with_profit(sales_rep_performance_pd_df):
    ...
    # Step 1: Define a colormap for different shades of Green
    cmap = get_cmap('Greens')
...
def plot_interaction_counts_by_country(data):
    ...
    colors = cmap(np.linspace(0.4, 1, num_interactions))  # Generate shades of Green
    ...
    # Plot bars for each interaction type with different shades of Green
    bars = []
